...
public class TuLiaoShenActivity extends Activity implements View.OnClickListener{
    ...
    private static List<LiaoShen> liaoShens = new ArrayList<LiaoShen>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tu_liao_shen);

        lv = (MyListView) findViewById(R.id.list_view_ls);
        ...
        listView_Comment = (ListView) findViewById(R.id.list_view_ls);
    ...
    private void submit() {
        ...
        LiaoShen liaoShen = new LiaoShen();
        liaoShen.setUserName(Constants.QQParams.Name);
        if(liaoShen.getUserName()!=null) {
            if (content.equals("")) {
                return;
            }
            liaoShen.setUserComment(content);
            liaoShen.save(TuLiaoShenActivity.this, new SaveListener() {
                @Override
                public void onSuccess() {
                    Toast.makeText(TuLiaoShenActivity.this, "评论成功", Toast.LENGTH_SHORT).show();

                }

                @Override
                public void onFailure(int i, String s) {
                    Toast.makeText(TuLiaoShenActivity.this, "评论失败", Toast.LENGTH_SHORT).show();
                }
            });

        }else {
            Toast.makeText(TuLiaoShenActivity.this, "请登录之后再进行评论!",Toast.LENGTH_SHORT).show();
            ed_comment.setText("");
        }
    }

    private void select() {
        BmobQuery<LiaoShen> Query = new BmobQuery<LiaoShen>();
        Query.order("-createdAt");
        Query.findObjects(TuLiaoShenActivity.this, new FindListener<LiaoShen>() {
            @Override
            public void onSuccess(List<LiaoShen> list) {
                liaoShens = list;
    ...
    public static class MyAdpter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            LiaoShen com = liaoShens.get(position);
            //获取用户id
